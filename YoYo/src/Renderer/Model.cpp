...
namespace yoyo
{
	...
	YAPI Ref<Model> ResourceManager::Load<Model>(const std::string& path)
	{
		...
		auto model_it = m_model_cache.find(id);
		if (model_it != m_model_cache.end())
		{
		...
		const auto& model = Model::LoadFromAsset(path.c_str());
		if (!model)
		{
			return nullptr;
		}
	...
	Ref<Model> Model::LoadFromAsset(const char* asset_path)
	{
		hro::Model hro_model = {};
